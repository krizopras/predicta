...
class RealisticScorePredictor:
    """
    Temel Skor Tahmin Sistemi - Base class
    """
    ...
    def _calculate_odds_penalty(self, odds_1: float, odds_2: float, odds_x: float) -> float:
        """Oran farkına göre dinamik penaltı hesapla"""
        odds_diff = abs(odds_1 - odds_2)
        odds_balance = min(odds_1, odds_2) / max(odds_1, odds_2)
        
        base_penalty = np.clip(1 - (odds_diff / 3.0), 0.08, 0.95)
        balance_bonus = 0.1 * odds_balance
        
        return np.clip(base_penalty + balance_bonus, 0.10, 0.98)

    def _ml_prediction(self, match_data: Dict, ms_pred: str) -> Tuple[str, List[Dict]]:
        """ML tabanlı tahmin - Geliştirilmiş oran penaltısı ile"""
        if self.model is None or self.space is None:
    ...
    def predict_with_details(self, match_data: Dict, ms_pred: str = "1") -> Dict[str, Any]:
        ...
        alternatives = [{"score": final_score, "prob": 0.0}]
...
class EnhancedRealisticScorePredictor(RealisticScorePredictor):
    """
    Geliştirilmiş Skor Tahmin Sistemi v5.0
    - Zamansal İstatistikler
    - Gelişmiş Metrikler
    - Poisson Simülasyonu
    - Açıklamalı Güven Skoru
    """

    LEAGUE_STATS_VERSION = "v5.0"
    ...
    def _calculate_expected_goals_enhanced(self, match_data: Dict) -> Tuple[float, float]:
        """Geliştirilmiş beklenen gol hesaplama"""
        try:
            feats = self._safe_feature_extraction(match_data)
            league = self._auto_detect_league(match_data)
            ...
            trend_weight = min(1.0, match_count / 300)
            ...
            lambda_h = (
                home_avg_adj * 0.35 +
                home_avg_feat * 0.25 +
                home_form * home_strength * 1.3 * 0.40
            ) * goal_calibration + stats["home_advantage"]

            lambda_a = (
                away_avg_adj * 0.35 +
                away_avg_feat * 0.25 +
                away_form * away_strength * 1.1 * 0.40
            ) * goal_calibration
    ...
    def _simulate_score_distribution(self, lambda_h: float, lambda_a: float, n_simulations: int = 5000) -> Dict[str, float]:
        """Poisson dağılımı ile skor olasılıklarını simüle et"""
        score_counts = defaultdict(int)
        
        for _ in range(n_simulations):
            h_goals = self.rng.poisson(lambda_h)
            a_goals = self.rng.poisson(lambda_a)
            
            h_goals = min(h_goals, self.max_goals)
            a_goals = min(a_goals, self.max_goals)
            
            score_counts[f"{h_goals}-{a_goals}"] += 1
        
        return {score: count / n_simulations for score, count in score_counts.items()}

    def _get_top_scores_from_distribution(self, distribution: Dict[str, float], ms_pred: str, top_n: int = 5) -> List[Dict]:
        """Dağılımdan en olası skorları çıkar"""
        valid_scores = {score: prob for score, prob in distribution.items() if self._ms_ok(ms_pred, score)}
        
        sorted_scores = sorted(valid_scores.items(), key=lambda x: x[1], reverse=True)[:top_n]
        
        return [
            {
                "score": score, 
                "prob": prob, 
                "confidence_level": "yüksek" if prob > 0.10 else "orta" if prob > 0.05 else "düşük"
            }
            for score, prob in sorted_scores
        ]

    def explain_confidence(self, confidence: float, match_data: Dict, stats: Dict, metrics: Dict) -> Dict[str, Any]:
        """Güven skorunu detaylı açıkla"""
        if confidence >= 0.80:
            level, emoji = "Çok Yüksek", "🟢"
        ...
        if match_count > 500:
            factors.append("✓ Zengin veri seti")
        elif match_count < 100:
            factors.append("⚠ Sınırlı veri")
        
        tempo = metrics.get("tempo_coefficient", 0.5)
        if tempo > 0.7:
            factors.append("⚡ Yüksek tempo")
        elif tempo < 0.3:
            factors.append("🐌 Düşük tempo")
        
        home_trend = abs(stats.get("home_trend", 0.0))
        away_trend = abs(stats.get("away_trend", 0.0))
        if home_trend < 0.1 and away_trend < 0.1:
            factors.append("📊 Kararlı trend")
        else:
            factors.append("📈 Değişken trend")
        ...
        if odds_diff < 0.5:
            factors.append("⚖️ Dengeli maç")
        elif odds_diff > 1.5:
            factors.append("⚡ Tek taraflı")
    ...
    def _calculate_confidence_enhanced(self, lam_h: float, lam_a: float, match_data: Dict) -> float:
        ...
        try:
            ...
            match_count = stats.get("match_count", 100)
            stability = min(1.0, match_count / 200)
            ...
            tempo_confidence = 0.9 + 0.2 * tempo_coefficient
    ...
    def predict_with_enhanced_details(self, match_data: Dict, ms_pred: str = "1") -> Dict[str, Any]:
        """Geliştirilmiş tahmin - Poisson simülasyonu & açıklamalı güven ile"""
        original_league = match_data.get("league", "Belirtilmemiş")
        detected_league = self._auto_detect_league(match_data)
        ...
        score_distribution = self._simulate_score_distribution(ensemble_h, ensemble_a, self.simulation_iterations)
        top_scores = self._get_top_scores_from_distribution(score_distribution, ms_pred, top_n=5)
        
        predicted_score = top_scores[0]["score"] if top_scores else f"{int(round(ensemble_h))}-{int(round(ensemble_a))}"
        
        confidence = self._calculate_confidence_enhanced(ensemble_h, ensemble_a, match_data)
        confidence_details = self.explain_confidence(confidence, match_data, stats, metrics)
        ...
        result = {
            ...
            "score_distribution": {k: round(v, 4) for k, v in sorted(score_distribution.items(), key=lambda x: x[1], reverse=True)[:10]},
            "league_info": {
                "original": original_league,
                "detected": detected_league,
                "auto_detected": detected_league != original_league,
        ...
        logger.info(
            f"[{home}–{away}] λ=({ensemble_h:.2f},{ensemble_a:.2f}) | "
            f"MS={ms_pred} → {predicted_score} | "
            f"{confidence_details['explanation']}"
        )
...
if __name__ == "__main__":
    ...
    print("🚀 GELİŞTİRİLMİŞ SKOR TAHMİN SİSTEMİ v5.0")
    print("=" * 80)
    print("✨ Yeni Özellikler:")
    print("   • Poisson tabanlı skor dağılım simülasyonu")
    print("   • Dinamik oran penaltısı")
    print("   • Açıklamalı güven skoru")
    print("   • Zamansal ağırlıklı istatistikler")
    print("   • Gelişmiş lig metrikleri")
    print("=" * 80 + "\n")
    ...
    print(f"🎲 Simülasyon: {sys_info['simulation_iterations']} iterasyon")
    print(f"⏱️ Zamansal istatistikler: {'Aktif' if sys_info['temporal_stats_enabled'] else 'Pasif'}")
    ...
    for i, match in enumerate(test_matches, 1):
        print(f"\n{'='*80}")
        print(f"🎯 GELİŞMİŞ TEST {i}: {match['home_team']} vs {match['away_team']}")
        print(f"{'='*80}")
        
        result = enhanced_predictor.predict_with_enhanced_details(match, "1")
        
        # Ana tahmin
        print(f"\n📊 ANA TAHMİN: {result['predicted_score']}")
        print(f"   Beklenen Goller: {result['expected_goals']['home']:.2f} - {result['expected_goals']['away']:.2f}")
        
        # Güven skoru detayları
        conf = result['confidence']
        print(f"\n{conf['emoji']} GÜVEN SKORU: {conf['level']} ({conf['score']:.1%})")
        print(f"   Faktörler:")
        for factor in conf['factors']:
            print(f"      {factor}")
        
        # En olası skorlar (Poisson simülasyonu)
        print(f"\n🎲 EN OLASI SKORLAR (Poisson Simülasyonu):")
        for idx, score_data in enumerate(result['top_scores'], 1):
            conf_icon = "🟢" if score_data['confidence_level'] == "yüksek" else "🟡" if score_data['confidence_level'] == "orta" else "🟠"
            print(f"   {idx}. {score_data['score']:5s} → {score_data['prob']:.1%} {conf_icon} ({score_data['confidence_level']})")
        
        # Skor dağılımı (Top 10)
        print(f"\n📈 SKOR DAĞILIMI (İlk 10):")
        for score, prob in list(result['score_distribution'].items())[:10]:
            bar_length = int(prob * 50)
            bar = "█" * bar_length
            print(f"   {score:5s} │{bar:50s}│ {prob:.2%}")
        ...
        if league_info['auto_detected']:
            print(f"\n🔍 LİG TESPİTİ: '{league_info['original']}' → '{league_info['detected']}'")
        else:
            print(f"\n🏆 Lig: {league_info['detected']}")
        
        print(f"\n📊 LİG İSTATİSTİKLERİ:")
        print(f"   Ortalama Skorlar: {stats['home_avg']:.2f} - {stats['away_avg']:.2f} (Toplam: {stats['total_avg']:.2f})")
        print(f"   Ev Sahibi Avantajı: {stats['home_advantage']:+.2f}")
        print(f"   Trendler: Ev {stats['home_trend']:+.3f}, Deplasman {stats['away_trend']:+.3f}")
        print(f"   Maç Sayısı: {stats['match_count']}")
        
        print(f"\n📈 GELİŞMİŞ METRİKLER:")
        print(f"   ⚡ Tempo Katsayısı: {metrics['tempo_coefficient']:.2f}")
        print(f"   🎯 Favori Galibiyet Oranı: {metrics['favorite_win_rate']:.2f}")
        print(f"   ⚽ Gol Verimliliği: {metrics['scoring_efficiency']:.2f}")
        print(f"   🛡️  Savunma Stabilitesi: {metrics['defensive_stability']:.2f}")
        ...
        print(f"   Simülasyon: {sys['simulation_count']} iterasyon")
        print(f"   Zamansal İstatistikler: {'✓' if sys['temporal_stats_enabled'] else '✗'}")
        print(f"   Gelişmiş Metrikler: {'✓' if sys['advanced_metrics_enabled'] else '✗'}")
    
    print(f"\n{'='*80}")
    print("✅ TÜM TESTLER TAMAMLANDI")
    print("="*80)
    print("\n💡 ÖNERİLER:")
    print("   1. Gerçek maç sonuçlarıyla karşılaştırarak doğruluk oranını ölç")
    print("   2. Adaptif lambda kalibrasyonu için tahmin geçmişi kaydet")
    print("   3. Farklı liglerde performans analizi yap")
    print("   4. Poisson simülasyon sayısını optimize et (hız vs. doğruluk)")
    print("   5. Güven skoru eşiklerine göre bahis stratejisi belirle")